...
for i in $(seq 1 "$pos_test_files"); do
    ...
    "${test_dir_scripts}/pos_case.sh" "$file_stream_in" "$file_stream_out_expect"
    if "${test_dir_scripts}/pos_case.sh" "$file_stream_in" "$file_stream_out_expect"; then
...
for i in $(seq 1 "$neg_test_files"); do
    ...
    if "${test_dir_scripts}/neg_case.sh" "$file_stream_in"; then
